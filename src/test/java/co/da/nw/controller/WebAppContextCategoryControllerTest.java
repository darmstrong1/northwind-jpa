...
import co.da.nw.config.TestUtil;
...
public class WebAppContextCategoryControllerTest {
    ...
    public void testShowCreateCategoryForm() throws Exception {
        mockMvc.perform(get("/category/create"))
            ...
            .andExpect(model().attribute(CategoryController.MODEL_ATTIRUTE_CATEGORY, hasProperty("categoryId", nullValue())))
            .andExpect(model().attribute(CategoryController.MODEL_ATTIRUTE_CATEGORY, hasProperty("categoryName", isEmptyOrNullString())))
            .andExpect(model().attribute(CategoryController.MODEL_ATTIRUTE_CATEGORY, hasProperty("description", isEmptyOrNullString())));
//            .andExpect(model().attribute(CategoryController.MODEL_ATTIRUTE_CATEGORY, hasProperty("picture", nullValue())));
        
        verifyZeroInteractions(serviceMock);
    }
    
    @Test
    public void add_EmptyCategory_ShouldRenderFormViewAndReturnValidationErrorForName() throws Exception {
        ...
        .andExpect(view().name(CategoryController.CATEGORY_ADD_FORM_VIEW))
        .andExpect(forwardedUrl("/WEB-INF/jsp/category/create.jsp"))
        .andExpect(model().attributeHasFieldErrors(CategoryController.MODEL_ATTIRUTE_CATEGORY, "categoryName"))
        .andExpect(model().attribute(CategoryController.MODEL_ATTIRUTE_CATEGORY, hasProperty("categoryId", nullValue())))
        .andExpect(model().attribute(CategoryController.MODEL_ATTIRUTE_CATEGORY, hasProperty("categoryName", isEmptyOrNullString())))
        .andExpect(model().attribute(CategoryController.MODEL_ATTIRUTE_CATEGORY, hasProperty("description", isEmptyOrNullString())));
    ...
    @Test
    public void add_DescriptionAndNameAreTooLong_ShouldRenderFormViewAndReturnValidationErrorsForNameAndDescription() throws Exception {
        String name = TestUtil.createStringWithLength(Category.MAX_LENGTH_CATEGORY_NAME + 1);
        ...
        .andExpect(forwardedUrl("/WEB-INF/jsp/category/create.jsp"))
        .andExpect(model().attributeHasFieldErrors(CategoryController.MODEL_ATTIRUTE_CATEGORY, "categoryName"))
        .andExpect(model().attributeHasFieldErrors(CategoryController.MODEL_ATTIRUTE_CATEGORY, "description"))
        .andExpect(model().attribute(CategoryController.MODEL_ATTIRUTE_CATEGORY, hasProperty("categoryId", nullValue())))
        .andExpect(model().attribute(CategoryController.MODEL_ATTIRUTE_CATEGORY, hasProperty("categoryName", is(name))))
        .andExpect(model().attribute(CategoryController.MODEL_ATTIRUTE_CATEGORY, hasProperty("description", is(description))));
    ...
    public void add_NewCategory_ShouldAddCategory() throws Exception {
        ...
        String expectedRedirectViewPath = TestUtil.createRedirectViewPath(CategoryController.REQUEST_MAPPING_LIST);
    ...
    @Test
    public void deleteById_CategoryFound_ShouldDeleteCategory() throws Exception {
        String name = "Fruits";
        String desc = "Various fruits";
        long id = 1L;
        
        Category deleted = new Category.Builder().setDescription(desc).build(name);
        ReflectionTestUtils.setField(deleted, "categoryId", id);
        
        when(serviceMock.delete(id)).thenReturn(deleted);

        String expectedRedirectViewPath = TestUtil.createRedirectViewPath(CategoryController.CATEGORY_LIST_VIEW);
